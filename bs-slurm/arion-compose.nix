...

{
  services = {
    control = addCommon {
      ...
      nixos.configuration.services.bs-slurm  = {server.enable = true;} // slurmconfig;
    }; 
    
    submit = addCommon {
      service.hostname="submit";
      nixos.configuration.services.bs-slurm  = { enableStools = true;} // slurmconfig;
    };
  } // multiple-services "node" (name: nodeId: (
    addCommon {
      service.hostname= name;
      nixos.configuration.services.bs-slurm  = {
        client = {enable = true; nodeId = nodeId; rangeId = vnodeFactor;}; } // slurmconfig;
    }));
}
